...
import (
	...
	"github.com/LMF709268224/titan-vps/api/client"
...
func GetTransactionAPI(ctx *cli.Context) (api.Transaction, jsonrpc.ClientCloser, error) {
	addr, headers, err := GetRawAPI(ctx, repo.Transaction, "v0")
	if err != nil {
		return nil, nil, err
	}

	if IsVeryVerbose {
		_, _ = fmt.Fprintln(ctx.App.Writer, "using transaction API v0 endpoint:", addr)
	}

	a, c, e := client.NewTransaction(ctx.Context, addr, headers)
	v, err := a.Version(ctx.Context)
	if err != nil {
		return nil, nil, err
	}

	if !v.APIVersion.EqMajorMinor(api.TransactionAPIVersion0) {
		return nil, nil, xerrors.Errorf("Remote API version didn't match (expected %s, remote %s)", api.TransactionAPIVersion0, v.APIVersion)
	}

	return a, c, e
}

func GetMallAPI(ctx *cli.Context) (api.Mall, jsonrpc.ClientCloser, error) {
	addr, headers, err := GetRawAPI(ctx, repo.Mall, "v0")
	if err != nil {
		return nil, nil, err
	}

	if IsVeryVerbose {
		_, _ = fmt.Fprintln(ctx.App.Writer, "using mall API v0 endpoint:", addr)
	}

	a, c, e := client.NewMall(ctx.Context, addr, headers)
	v, err := a.Version(ctx.Context)
	if err != nil {
		return nil, nil, err
	}

	if !v.APIVersion.EqMajorMinor(api.MallAPIVersion0) {
		return nil, nil, xerrors.Errorf("Remote API version didn't match (expected %s, remote %s)", api.MallAPIVersion0, v.APIVersion)
	}

	return a, c, e
}

func APIGate(ctx *cli.Context) (api.Mall, jsonrpc.ClientCloser, error) {
	addr, headers, err := GetRawAPI(ctx, repo.Mall, "v0")
	if err != nil {
		return nil, nil, err
	}
